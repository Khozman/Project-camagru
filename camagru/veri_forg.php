...
if (isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['code']) && !empty($_GET['code']))
{
    ...
    if ($row_no > 0)
    {
        ...
        header("refresh:0.01; url=reset_pass.php?email=$email");
    }
    else
    {
        echo '<script> alert ("Check your email for RESET link!")</script>';
    }
    }
    else {
    echo '<script> alert ("Please use link sent to email to RESET password!")</script>';
}
